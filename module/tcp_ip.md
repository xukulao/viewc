...
- TCP协议
   - 特点
   双全工，在连接上进行读写操作完成通信，基于数据流传输，通信前先进行连接，数据交换完成，后断开
   释放系统资源

   - 字节流
   发送的写操作次数和接收端的读操作次数没有任何关系，上层的应用程序对数据的发送和接收没有边界限制

   UDP则是相反

   - TCP以应答机制发送数据，每一次发送TCP报文都要得到接收的应答，同时会启用超时重传机制，数据发送
   后会启动定时器，未在指定时间应答就会重新发送，由于低层协议是IP协议，数据接受到的是混乱，所以会重新
   整理，编排再传递给应用层
